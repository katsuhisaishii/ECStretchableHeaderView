...
- (void)_scrollView:(UIScrollView *)scrollView offsetChanged:(NSDictionary *)change
{
    ...
    if( self.minHeight + heightCheck > self.maxHeight ){
        self.heightConstraint.constant = self.maxHeight;
    }
    else if( self.minHeight + heightCheck < self.minHeight ){
        self.heightConstraint.constant = self.minHeight;
    }
    else{
        self.heightConstraint.constant = self.minHeight + heightCheck;
    }
    
    //    NSLog(@"scrollView:%@       oldYOffset:%@       newYOffset:%@       elativePosition:%@      heightCheck:%@      constant:%@",
    //          NSStringFromCGRect(scrollView.bounds),@(oldYOffset),@(newYOffset),@(relativePosition),@(heightCheck),@(self.heightConstraint.constant));
}
...
- (void)_scrollView:(UIScrollView *)scrollView sizeChanged:(NSDictionary *)change
{
    ...
    contentInset.top = self.maxHeight - self.minHeight;
